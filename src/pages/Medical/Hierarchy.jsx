  ...
  return (
    <div className={cx("mb-2")}>
      ...
      <EntityActionBar
        ...
        className={cx("flex items-center justify-between rounded-2xl", " py-3.5 px-5 text-white")}
        actionClassName={cx("bg-white/20 rounded-lg border border-white/40")}
        style={{ background: groupColor.gradient }}
      >
        ...
        <div className="ml-[20px] mt-[6px] space-y-[6px]">
          {departments.map((dept, idx) => {
            ...
            return (
              <div
                key={dept.id}
                className={cx("fadeUp", "overflow-hidden rounded-xl")}
                style={{ boxShadow: "var(--shadow)", animationDelay: `${Math.min(idx * 40, 400)}ms` }}
              >
                <div className={cx()}>
                  <DeptActionBar
                    groupId={group.id}
                    ...
                    style={{ color: isMatched(dept.name, keyword) && groupColor.text }}
                  ...
                  >
                    <div className={cx("flex items-center gap-2")}>
                      ...
                      <Item
                        as="div"
                        icon={<DeptIcon />}
                        className={cx("flex items-center justify-center", "text-[13px] w-[30px] h-[30px] rounded-lg")}
                        style={{
                          color: groupColor.solid,
                  ...
                  {expandedDepartments[dept.id] && (
                    <div className="px-4 py-2 space-y-[4px] bg-white">
                        ...
                        dept.specialties.map((spec, idx) => (
                          <SpecActionBar
                            ...
                            style={{
                              ...
                              backgroundColor: `color-mix(in srgb, ${groupColor.light} 30%, white)`,
                            }}
                          >
                            <div className="flex items-center gap-2">
                              <div className={cx("w-1 h-1 rounded-full", "bg-[var(--color-primary-700)]")} />
                              <Item
                                as="span"
                                children={spec.name}
                                itemClassName={cx("text-[12.5px] font-bold")}
                                style={{ color: isMatched(spec.name, keyword) && groupColor.text }}
                              />
                            </div>
                          </SpecActionBar>
                        ))
